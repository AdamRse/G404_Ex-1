...
#atk_min_max=attaque.sort_values()
...
axs[1,0].set_title('Potentiel attaque / defense')
...
df["Mega"]=df['Name'].str.contains(r'^mega\s.+$', case=False, regex=True)
#print(df[df['Mega'] == True])
lengendaires = df.loc[df['Legendary'] == True, 'Total']
mega=df.loc[df['Mega'] == True, 'Total']
normaux = df.loc[(df['Legendary'] == False) & (df['Mega'] == False), 'Total']

axs[1,1].boxplot(
    [normaux, mega, lengendaires],
    tick_labels=['Normaux', 'Mega', 'Legendaires'],
    showmeans=True,
)
axs[1,1].text(0.5, 0, f"Attention : Les pokémon suivants sont Légendaires ET Mega :\n{', '.join([x for x in df.loc[(df['Legendary'] == True) & (df['Mega'] == True), 'Name']])}", color="red")
axs[1,1].grid(axis="y", alpha=0.6)
axs[1,1].set_title('Puissance totale')
# --------------

fig.suptitle("Statistiques pokémon")
fig.show()
#fig.savefig('output/imgPokeGraph1.png')
