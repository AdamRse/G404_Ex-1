...
Le [notebook de préparation](../notebooks/Preparation_projet_individuel_FR.ipynb)
montre les cinq étapes sur un petit exemple en dix minutes.
Cette fiche reste le support détaillé à consulter pendant votre projet.

## Utiliser la fiche

Créez `essais_snippets.py` dans `lab/pipeline_template/`, puis utilisez le Python `.venv`
du cours. Ajoutez les extraits dans l'ordre. Ils écrivent uniquement dans
`lab/outputs/exemples_snippets/`. Vous pouvez lancer le fichier après chaque
